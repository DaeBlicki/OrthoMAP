/**
 * @file OrthoMAP_toolbox.h
 * @brief High-performance C++ interface for R via Rcpp
 * 
 * Ideal for pipelines needing cross-species expression alignment, gene family
 * collapsing, or evolutionary transcriptomics integration.
 *
...
#ifndef ORTHOMAP_TOOLBOX_H
#define ORTHOMAP_TOOLBOX_H
...
// ============================================================================
// Utility functions usefull in all steps of the pipeline
// Functions:
//      - print_current_time
//      - print_progress
//      - name_to_idx_map
//      - gene_to_orthogroup_map
// ============================================================================

// return current time as string
// [[Rcpp::export]]
Rcpp::CharacterVector print_current_time() {
  time_t timestamp = time(NULL);
  struct tm datetime = *localtime(&timestamp);
  return std::asctime(&datetime);
}

// create hashtable mapping orthogroup.name to orthogroup.idx
// [[Rcpp::export]]
std::unordered_map<std::string, int> name_to_idx_map(Rcpp::IntegerVector input) {
    std::unordered_map<std::string, int> hashtable;
    const Rcpp::CharacterVector names = input.names();
...
// create hashtable mapping gene.name to orthogroup.name
// [[Rcpp::export]]
std::unordered_map<std::string, std::string> gene_to_orthogroup_map(Rcpp::CharacterVector input) {
    std::unordered_map<std::string, std::string> hashtable;
    Rcpp::CharacterVector names = input.names();

    for (int i = 0; i < input.size(); ++i) {
        std::string gene = Rcpp::as<std::string>(names[i]);
        std::string orthogroup = Rcpp::as<std::string>(input[i]);
        hashtable[gene] = orthogroup;
    }
...
/**
 * @name: convert_gene_to_oma_expression
 * @brief: convert sparse gene expression matrix of a species to 
 *         the corresponding orthogroup expression matrix.
 * @param gene_expression_matrix: dgCMatrix, intra-species genes to cells
 * @param genes_names: CharacterVector, names of the genes in the cells
 * @param orthogroup_names: CharacterVector, name of the orthogroups
 * @param lookup_gene_to_orthogroup: Hashtable, project gene to orthogroup
 * @param lookup_orthogroup_to_idx: Hashtable, project orthogroup to index
 */
...
// Access from progress library
// accessed at 15/08/25 from GitHub
// 
namespace RProgress {
